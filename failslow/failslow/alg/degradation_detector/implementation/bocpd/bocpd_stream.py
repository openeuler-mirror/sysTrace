"""Stream implementation of Bayesian Online Change Point Detection (BOCPD) algorithm."""
...
class Probability1d(ProbabilityRecord, metaclass=DistributionMeta):
    """
    记录所有从last n的distribution的结果, n=N,N-1,....,0
    """
...
class GaussianProb1d(Probability1d):
    ...
    def get_probability(self, obs: float) -> np.ndarray:
        ...
        sigma = np.sqrt(
            (self._sumX_sqr / self._N)
            - np.square(self._sumX / self._N)
            + self._init_sigma2
        )
...
class StudentTProb1d(Probability1d):
    ...
    def update(self, obs: float):
        self._betaAll = np.append(
            self._countAll
            + self._countAll / (self._countAll + 1) * np.square(obs - self._muAll) / 2,
            1,
        )
        self._muAll = np.append(
            (self._muAll * self._countAll + obs) / (self._countAll + 1), obs
        )
    ...
    def get_probability(self, obs: float) -> np.ndarray:
        ...
        precision = (
            self._alphaAll * self._countAll / (self._countAll + 1) / self._betaAll
        )
...
class LinearProb1d(Probability1d):
    ...
    def __init__(self, init_sigma2=10.0):
        self._init_sigma2 = init_sigma2
...
class LinearProbRobust(Probability1d):
    """
    线性模型的鲁棒概率估计，采用Huber损失估计方差，提升对异常点的鲁棒性。
    """

    def __init__(self, init_sigma2=1.0, huber_delta=1.0):
        self._init_sigma2 = init_sigma2
    ...
    def get_sigma(self, a: np.ndarray = None, b: np.ndarray = None):
        ...
        if n == 0:
            return np.sqrt(self._init_sigma2)
        ...
        sigma2 = np.mean(huber_loss) + self._init_sigma2
    ...
    def get_probability(self, obs: float) -> np.ndarray:
        ...
        pred = a * idx + b
        mu = pred
        probs = stats.norm.pdf(obs, mu, sigma) + 1e-10
        return probs
...
class LinearProb1dWithRunLengthBonus(LinearProb1d):
    def get_probability(self, obs):
        probs = super().get_probability(obs)
        # 设置run-length上界，防止奖励过大
        capped_N = np.minimum(self._N, 100)
        bonus = 1.01**capped_N  # 奖励因子，可调整
        probs *= bonus
        return probs
...
class MTSProbIID(ProbabilityRecord):
    ...
    @classmethod
    def define_iid_mts_distribution(
        cls, distribution_1d_type: type[Probability1d], n_dims: int
    ):
...
class BOCPDStreamConfig(BaseModel):
    ...
    def validate_distribution(self) -> "BOCPDStreamConfig":
        if isinstance(self.distribution, str):
            if self.distribution not in DistributionMeta.registry:
                err_msg = (
                    f"Unsupported distribution type: {self.distribution}. "
                    f"Supported types: {list(DistributionMeta.registry.keys())}"
                )
...
class BOCPDStream:
    def __init__(self, config: BOCPDStreamConfig, *args, **kwargs):
        # P(r_0=0)=1
        self._P_r_t = np.ones(0, dtype=np.float64)
        self._dist = DistributionMeta.registry[config.distribution]()
        logger.debug(
            "Initialized BOCPDStream with distribution: %s", config.distribution
        )
        self._h_lambda = config.h_lambda
        self._ignore_prop_lb = config.ignore_prop_lb
        self._egress_distance = config.egress_distance
        self._probs = np.ones(0, dtype=np.float64)
        self.obs = np.zeros(0, dtype=np.float64)
        self._record_probs = config.record_probs
        self._enable_crop_acceleration = config.enable_crop_acceleration
...
__all__ = ["BOCPDStreamConfig", "BOCPDStream", "ProbabilityRecord", "Probability1d"]
