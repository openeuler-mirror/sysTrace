...
from ...detector import DegradationAlert, OnlineDegradationDetector, DetectorConfig
...
class WindowState(BaseModel):
    ...
    def update_value(self, value: float):
        ...
        self.window_sum_sqr += value * value
        self.count += 1
    ...
    def get_mu_sigma(self) -> tuple[float, float]:
        if self.count == 0:
            return 0.0, 0.0
        mean = self.window_sum / self.count
        variance = self.window_sum_sqr / self.count - mean * mean
...
class SlidingWindowKSigmaNaive(OnlineDegradationDetector):
    """
    ...
    def update(self, obs, timestamp: Optional[int] = None):
        ...
        #logger.debug("index: %d, obs: %.4f, dev: %.4f, mean: %.4f, std: %.4f, ub: %.4f, lb: %.4f, signal: %d",
        #             self.window_state.index_counter, obs, diviation, mean, std, ub, lb, anom_signal)
...
__all__ = ["SlidingWindowKSigmaNaive"]
