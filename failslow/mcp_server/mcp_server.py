...
@mcp.prompt(title="self_introduction",
            description="工具定位")
def self_introduction() -> str:
    return "面向运维、开发人员，支持自然语言对接，实现启发式调优，实现3个工具接口，分别为性能劣化感知工具，慢卡定界工具，报告输出工具。"


@mcp.prompt(title="劣化感知",
            description="调用逻辑:1. 当用户询问特定任务ID的机器性能是否劣化时调用。2. 检测结果将决定后续流程走向。\
...
@mcp.tool(name="slow_node_detection_tool",
          title="检查工具：当检测到性能劣化时诊断具体问题点,即定界工具")
...
@mcp.prompt(title="报告生成工具",
            description="调用slow_node_perception_tool 或 slow_node_detection_tool 后把结果传入generate_report ")
@mcp.tool()
