...
bool LogStream::setLogFile(const std::string &file_path) {
    ...
    if (!dir.empty() && !std::filesystem::exists(dir)) {
        try { std::filesystem::create_directories(dir); } catch (...) { return false; }
    }
...
void closeLoggingFile() {
    log::getLogStream().closeLogFile();
}
} // namespace systrace

extern "C" {
static void systrace_log_impl(LogLevel level, const char *module, const char *format, va_list args) {
    char buffer[4096];
...
void systrace_log_info(const char *module, const char *format, ...) {
    va_list args; va_start(args, format); systrace_log_impl(INFO, module, format, args); va_end(args);
}
void systrace_log_warning(const char *module, const char *format, ...) {
    va_list args; va_start(args, format); systrace_log_impl(WARNING, module, format, args); va_end(args);
}
void systrace_log_error(const char *module, const char *format, ...) {
    va_list args; va_start(args, format); systrace_log_impl(ERROR, module, format, args); va_end(args);
}
void systrace_log_fatal(const char *module, const char *format, ...) {
    va_list args; va_start(args, format); systrace_log_impl(FATAL, module, format, args); va_end(args);
}
}