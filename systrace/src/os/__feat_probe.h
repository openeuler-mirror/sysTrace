 ...
 *Utility functions for feature probes
 ******************************************************************************/
...
/* BPF_MAP_TYPE_RINGBUF original defined in /usr/include/linux/bpf.h, which from
   kernel-headers if BPF_MAP_TYPE_RINGBUF wasn't defined, this kernel does not
   support using ringbuf */
#ifndef BPF_MAP_TYPE_RINGBUF
#define BPF_MAP_TYPE_RINGBUF                                                   \
    27 // defined here to avoid compile error in lower kernel version
...
static inline bool probe_ringbuf() {
    ...
    if ((map_fd = bpf_map_create(BPF_MAP_TYPE_RINGBUF, NULL, 0, 0,
                                 getpagesize(), NULL)) < 0) {
        return false;
    }
