...
bool FtracePlugin::start(const json &params, int duration) {
    if (systrace::util::config::GlobalConfig::Instance().local_rank != 0) return true;
    bool expected = false;
    ...
    try {
        ...
        if (!init_ftrace()) throw std::runtime_error("Ftrace init fail");
        ...
        for (int cpu_id: target_cpus) {
            ...
            std::string raw_path = ftrace_path_ + "per_cpu/cpu" + std::to_string(cpu_id) + "/trace_pipe";
            reader.raw_fd = open(raw_path.c_str(), O_RDONLY | O_NONBLOCK);

            std::string out_path = output_dir_ + "/cpu" + std::to_string(cpu_id) + ".raw";
            reader.out_fd = open(out_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);

            if (pipe(reader.pipe_fds) == -1) throw std::runtime_error("Pipe fail");
            fcntl(reader.pipe_fds[0], F_SETPIPE_SZ, K_SPLICE_SIZE);
            fcntl(reader.pipe_fds[1], F_SETPIPE_SZ, K_SPLICE_SIZE);

            if (reader.raw_fd < 0 || reader.out_fd < 0) throw std::runtime_error("Open per-cpu FD fail");
            cpu_readers_.push_back(std::move(reader));
        }
        for (auto &reader: cpu_readers_) {
            reader.worker = std::thread(&FtracePlugin::per_cpu_splice_loop, this, std::ref(reader));
        }
        if (duration > 0) {
            systrace::utils::TimerManager::getInstance().startTimer(get_id(), duration, [this]() { this->stop(); });
        }

        LOG_MODULE(INFO, pluginName_) << "Parallel Ftrace started. Output file: " << output_;
        return true;
...
void FtracePlugin::stop() {
    ...
    if (active_.load()) {
        try {
            ...
            for (auto &reader: cpu_readers_) {
                if (reader.worker.joinable()) reader.worker.join();
            }

            cleanup_resources();
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
            LOG_MODULE(DEBUG, pluginName_) << "Parallel ftrace stop. All CPU buffers flushed.";
            reset_ftrace();

            active_.store(false);
            LOG_MODULE(DEBUG, pluginName_) << "All collection threads joined. Starting data aggregation and processing...";
            process_raw_data();
...
void FtracePlugin::per_cpu_splice_loop(CpuReader &reader) {
    ...
    while (true) {
        ...
        if (collect_duration_ > 0) {
            ...
            if (elapsed >= collect_duration_) should_exit = true;
        }

        // 零拷贝：Raw FD -> Pipe In -> Pipe Out -> File FD
        ssize_t moved = splice(reader.raw_fd, nullptr, reader.pipe_fds[1], nullptr,
                               K_SPLICE_SIZE, SPLICE_F_MOVE | SPLICE_F_NONBLOCK);

        if (moved > 0) {
            splice(reader.pipe_fds[0], nullptr, reader.out_fd, nullptr, moved, SPLICE_F_MOVE);
        } else if (moved < 0) {
...
void FtracePlugin::process_raw_data() {
    ...
    std::ofstream outfile(output_, std::ios::out | std::ios::trunc | std::ios::binary);

    for (const auto& reader : cpu_readers_) {
        std::string part_path = output_dir_ + "/cpu" + std::to_string(reader.cpu_id) + ".raw";
...
bool FtracePlugin::init_ftrace() {
    ...
    PluginUtils::write_file(ftrace_path_ + "tracing_cpumask", PluginUtils::generate_cpu_mask(cpus));
    PluginUtils::write_file(ftrace_path_ + "buffer_size_kb", task_config_.core_config.buffer_size_kb);
    ...
    if (!task_config_.core_config.trace_functions.empty()) {
        PluginUtils::write_file(ftrace_path_ + "current_tracer", task_config_.core_config.function_tracer);
        PluginUtils::write_file(ftrace_path_ + "set_ftrace_filter", task_config_.core_config.trace_functions);
    }
    if (!task_config_.core_config.ftrace_pid.empty()) {
        PluginUtils::write_file(ftrace_path_ + "set_ftrace_pid", task_config_.core_config.ftrace_pid);
    }
    if (!task_config_.core_config.event_pid.empty()) {
        PluginUtils::write_file(ftrace_path_ + "set_event_pid", task_config_.core_config.event_pid);
    }
    PluginUtils::write_file(ftrace_path_ + "options/stacktrace", task_config_.core_config.stack_trace ? "1" : "0");
    PluginUtils::write_file(ftrace_path_ + "tracing_on", "1");
    return true;
}

void FtracePlugin::reset_ftrace() {
    if (ftrace_path_.empty()) return;

    try { PluginUtils::write_file(ftrace_path_ + "tracing_on", "0"); } catch (...) {}

    try { PluginUtils::write_file(ftrace_path_ + "events/enable", "0"); } catch (...) {}

    try { PluginUtils::write_file(ftrace_path_ + "set_ftrace_filter", ""); } catch (...) {}
    try { PluginUtils::write_file(ftrace_path_ + "set_graph_function", ""); } catch (...) {}

    try { PluginUtils::write_file(ftrace_path_ + "set_ftrace_pid", ""); } catch (...) {}

    try {
        PluginUtils::write_file(ftrace_path_ + "current_tracer", "nop");
    } catch (const std::exception &e) {
        LOG_MODULE(WARN, pluginName_) << "Final Tracer reset still busy. This usually means an unclosed FD exists. " << e.what();
    }

}

TraceTaskConfig FtracePlugin::parse_config_from_json(const nlohmann::json &params) {
    ...
    try {
        ...
        if (params.contains("events")) {
            ...
            if (events.is_array()) {
                ...
                core.enable_events = PluginUtils::split_string(events.get<std::string>());
            } else {
                throw std::invalid_argument("events type error");
            }
            core.enable_events.erase(
                    std::remove_if(core.enable_events.begin(), core.enable_events.end(),
        ...
        if (params.contains("event_stack_trace")) {
            core.stack_trace = PluginUtils::safe_parse_boolean(params.at("event_stack_trace"));
        }

        if (params.contains("func_stack_trace")) {
            core.func_stack_trace = PluginUtils::safe_parse_boolean(params.at("func_stack_trace"));
        }
        ...
        throw std::runtime_error("parse_config_from_json error");
    }
