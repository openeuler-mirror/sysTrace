...
def run_slow_node_perception(args: Dict, task_id: str):
    ...
    logger.info(
        f"Detection data range: {detecting_range_steps}, data queue: {len(DATA_QUEUE)}, drop data length: {DROP_DATA_LENGTH}.")
    detected_data = DATA_QUEUE.loc[(detecting_range_steps[0] - DROP_DATA_LENGTH): (
            detecting_range_steps[1] - DROP_DATA_LENGTH)].reset_index(drop=True)
